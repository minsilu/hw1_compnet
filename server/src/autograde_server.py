...
class TestServer:
    ...
    def build(self):
        ...
        while True:
            ...
            if stderr and not have_error:
                ...
                have_error = True
        if have_error: # if there are warnings, the credit will be deducted by major
            self.credit -= self.major

    def create_test_file(self, filename, filesize = 10000):
        '''
        This function creates a test file with random binary data
        Parameters:
            filename: The name of the file to be created
            filesize: The size of the file in bytes (default: 10000)
        The file is filled with random double-precision floating-point numbers
        '''
        f = open(filename, 'wb')
    ...
    def test_public(self, port=21, directory='/tmp'):
        '''
        This function tests the public part of the server
        Parameters:
            port: The port number to run the server on (default: 21)
            directory: The directory to run the server in (default: /tmp)
        The function tests the server's connection, login, SYST, TYPE, PORT download,
        PASV upload, and QUIT commands
        '''
        ...
        time.sleep(0.1) # sleep for 0.1 seconds to ensure the server is ready
        try:
            ftp = FTP()
            # connect
            ...
            if not filecmp.cmp(filename, directory + '/' + filename): # compare the uploaded file with the original file
